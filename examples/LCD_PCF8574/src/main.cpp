 ...
 * @brief AM4 PCF8574.ino (LCD_PCF8574) compat-macro port — content-
 *        identical to `I2C.ino` (LCD_Malpartida)'s own port (confirmed via
 *        a real diff against both vendored sources — same TOGGLE/SELECT/
 *        CHOOSE/subMenu/FIELD/EDIT hex/idle tree), differing only in real
 *        AM4 source by vendor library (`LiquidCrystal_PCF8574`, Matthias
 *        Hertel's driver, vs. `LiquidCrystal_I2C`) — same underlying
 *        hardware (HD44780 over a PCF8574 I2C backpack, addr 0x27, 16x2).
 *        See the original at github.com/neu-rah/ArduinoMenu
 *        examples/LCD_PCF8574/PCF8574/PCF8574.ino.
 *
 * Since `I2C.ino`'s own port already reuses OneIO's native `I2cLcd`
 * directly (not a vendor wrapper at all — that hardware is already fully
 * covered natively), this port is the exact same reuse, byte-for-byte:
 * the "different vendor library" distinction between the two real AM4
 * examples is moot here, since neither port depends on either vendor
 * library. See `.RnD/AM4check/LCD_PCF8574` for the real-hardware wiring
 * (identical to `LCD_Malpartida`'s own — same chip, same pins).
 *
...
int main() {
  ...
  //        7 Select, 8 Choose, 9 Alert test, 10 Hex, 11 <Back>
  ...
  printf("OK: PCF8574.ino (LCD_PCF8574) compat-macro port (native I2cLcd "
         "16x2 HD44780+PCF8574 over real encoder+button proven separately, "
         "see .RnD/AM4check/LCD_PCF8574) verified\n");
  return 0;
}
