 ...
 * @brief AM4 I2C.ino (LCD_Malpartida) compat-macro port — 16x2 I2C LCD
 *        (`LiquidCrystal_I2C` addr 0x27), content close to `ansiSerial.ino`/
 *        `serialio.ino`'s own shape (TOGGLE/SELECT/CHOOSE/subMenu/FIELD/
 *        EDIT hex/idle). See the original at github.com/neu-rah/
 *        ArduinoMenu examples/LCD_Malpartida/I2C/I2C/I2C.ino.
 *
 * Real design call: this targets the exact same hardware (HD44780 over a
 * PCF8574 I2C backpack, addr 0x27, 16x2) OneIO's own native `I2cLcd`
 * already fully supports and has real-hardware-verified since
 * `LiquidCrystal.ino`/`concurrent.ino`/`clickEncoder.ino`'s own ports —
 * reused directly (`oneMenu::LcdOut<Lcd>`), same "OneIO already covers
 * this exact chip, no new vendor wrapper needed" precedent
 * `SSD1306Ascii.ino`'s own port already established, not a new
 * `LiquidCrystal_I2C` vendor wrapper. See `.RnD/AM4check/LCD_Malpartida`
 * for the real-hardware wiring (encoder+button on the *same* pins the
 * real source itself uses — no pin changes needed).
 *
...
int main() {
  ...
  printf("OK: I2C.ino (LCD_Malpartida) compat-macro port (native I2cLcd "
         "16x2 HD44780+PCF8574 over real encoder+button proven separately, "
         "see .RnD/AM4check/LCD_Malpartida) verified\n");
  return 0;
}
